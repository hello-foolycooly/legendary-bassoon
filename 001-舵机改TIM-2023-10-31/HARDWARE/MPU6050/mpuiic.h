...
//警告的原因可能是因为 GPIOB->CRH 或 GPIOB->CRL 中的某些位的值被修改成了超出它们所能表示的范围的值。
//重新定义 sda_out 和 sda_in 的宏定义，使其产生的值不超出 GPIO 控制寄存器内部字段的范围。比如，可以使用移位运算来代替位偏移计算
#define MPU_SDA_IN()   {GPIOB->CRL &= ~(0xFU << 28); GPIOB->CRL |= (8U << 28);}
#define MPU_SDA_OUT()  {GPIOB->CRL &= ~(0xFU << 28); GPIOB->CRL |= (3U << 28);}
//每一组IO口有0~15一共16个端口
