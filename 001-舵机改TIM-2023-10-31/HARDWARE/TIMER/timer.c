...
void TIM3_PWM_Init(u16 arr, u16 psc)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE); //使能通用定时器2时钟，A0引脚对应TIM2CHN1
  ...
  GPIO_Init(GPIOB, &GPIO_InitStructure); //初始化引脚GPIOA0
	...
	TIM_OCInitTypeStrue.TIM_OutputState=TIM_OutputState_Enable; //使能PWM输出
	TIM_OCInitTypeStrue.TIM_Pulse = 0; //设置待装入捕获比较寄存器的脉冲值
//	TIM_OC1Init(TIM3, &TIM_OCInitTypeStrue); //根TIM_OCInitTypeStrue参数初始化定时器2通道1
...
//	TIM_OC2PreloadConfig(TIM2, TIM_OCPreload_Disable);
	...
	TIM_ARRPreloadConfig(TIM3, ENABLE); //CH1预装载使能
